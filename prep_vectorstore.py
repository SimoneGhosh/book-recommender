...
# Initialize embeddings for OpenRouter
embeddings = OpenAIEmbeddings(
    # 1. Point to OpenRouter instead of OpenAI
    base_url="https://openrouter.ai/api/v1",
    # 2. Use your OpenRouter API Key
    api_key=os.getenv("OPENAI_API_KEY"), 
    # 3. Specify an embedding model available on OpenRouter
    # Standard options: "openai/text-embedding-3-small" or "nomic-ai/nomic-embed-text-v1"
    model="openai/text-embedding-3-small", 
    # 4. (Optional) Required by some providers to avoid 403 errors
    default_headers={
        "HTTP-Referer": "http://localhost:3000", # Can be any URL
        "X-Title": "My Local RAG App",
    }
)
